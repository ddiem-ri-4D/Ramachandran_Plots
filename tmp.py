...
def main(pdb, itmod, model_num, itchain, chain_num, plot_type, out_dir, verb, save, file_type):
	...
	# print(itmod, model_num, itchain, chain_num)

	# Đường dẫn đến thư mục chứa các file pdb
	# pdb = "./TEST_FN/"
	folder_path = pdb

	# Khởi tạo một DataFrame trống để chứa dữ liệu từ các file pdb
	...
	for filename in os.listdir(folder_path):
		if filename.endswith(".pdb"):
			...
			# Thực hiện ExtractDihedrals và thêm dữ liệu vào DataFrame
			userpdb_df = ExtractDihedrals(pdb_file_name=pdb_path, iter_models=True, 
										model_number=0, iter_chains=True, chain_id=0)
			userpdb_df = userpdb_df.dropna()
			
			# Thêm dữ liệu vào DataFrame tổng
			combined_df = pd.concat([combined_df, userpdb_df])

	combined_df = combined_df.reset_index(drop=True)
	# combined_df.to_parquet("./data/combined_df.parquet")
